...
public:
	...
	//合成的移动操作
	//如果定义了自己的(不是默认的也不是合成的)拷贝构造函数、拷贝赋值运算符、析构函数中任何一个，编译器就不会生成合成的移动操作
	...
	//移动:不定义拷贝(自己的，非合成的，非默认的)且都能移动我才合成，否则我不合成，如果用=default强制我合成但又不都能移动，我就删除它
