...
//=================================================附录===========================================
//基础数据类型
//	类型		含义			最小尺寸	16位编译器	32位编译器	64位编译器           
//	bool		布尔类型		未定义		8		8		8
//	char		字符			8		8		8		8
//	short		短整形			16		16		16		16
//	int 		整形			16		16		32		32
//	long 		长整形			32		32		32		64
//	long long 	长整形			64		64		64		64	
//	float		单精度浮点数		6位有效数字	32		32		32
//	double		双精度浮点数		10位有效数字	64		64		64
//	long double	扩展精度浮点数		10有效数字	64		64		64	
//	指针		指针类型		--		16		32		64
//	wchar_t 	宽字符			16		--		--		--
...
//	* 指针类型: 是指向变量的地址，长度跟机器寻址有关，因此16机16bit,32位机32bit，64位机64bit
//	* sizeof会把字符串结束符\0计算进去，strlen不会
//
//内存对齐
//	* 结构体中的内存对齐，很重要，结构体的大小由结构体的元素和排列顺序有关，因此在定义结构体是注意这些细节可以节约内存开销
//	* 结构体元素按定义顺序一一放到内存中，但排列并不是紧密排列的；
//	1. 元素为基本数据类型: 从结构体的首地址开始，每个元素都会认为内存是以自己的大小来划分的，因此元素放置的起始位置一定是自己宽度的整数倍。
//	2. 包涵指针: 根据指针大小，按照基本数据类型规则
//	3. 包含其他结构体: 结构体放置的初始位置是结构体中最宽元素（基本数据类型/或指针）长度的整数倍
//	4. 内存大小需是所有元素中最宽元素（基本数据类型/或指针类型）长度的整数倍，不够则补齐。
//
//类的内存
//	* 规则:
//		1. 非静态成员变量总和
//		2. 加上内存对齐
//		3. 加上虚函数额外开支,多增加一个指针类型，在所有成员前面
//		4. 对于继承体系，基类+派生类
//	* 空类占一个字节
//	* 普通成员函数不占空间(非虚函数)
//	* 类有虚函数或虚继承，多一个指针成员大小，在所有数据成员前面
//	* 成员变量占用内存，需考虑内存对齐
//	* 当类的成员占用内存时,原先空类占用的1字节不再计算
//
//大小端模式
//	* 大端模式: 高位字节存在低位地址，如unsigned int value = 0x1234
//		`buf[0]=0x12 buf[1]=0x34`
//	* 小端模式: 低位字节存在低位地址，如unsigned int value = 0x1234
//		`buf[0]=0x24 buf[1]=0x12`
//	* 如何判断字节序
//		1. 转为char* 获取第一个字节
//			`int a=0x1234, char b = *((char*)&a), if(b==0x12){大端}`
//		2. 联合体: 联合不同变量共用一块内存，并且它们都是从低位地址开始存放
//			`union NUM {int a; char b;}num; num.a = 0x1234; if(num.b==0x12){大端}`
//
//函数
//	* 实参是形参的初始值
//	* 自动对象: 
